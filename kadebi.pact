...
(module kadebi GOVERNANCE
  ...
  (defun lock(mode:string)
    (enforce-keyset "free.admin-keyset")
    (update state-table mode {"locked": true})
  )
  (defun unlock(mode:string)
    (enforce-keyset "free.admin-keyset")
    (update state-table mode {"locked": false})
  )
  (defun enforce-current-round(mode:string round:integer)
    (let ((current-round (get-current-round mode)))
      (enforce (= round current-round) (format "{} is not current round!" [round]))
    )
  )
  (defun get-mode-key (mode:string)
    mode
  )
  (defun get-mode-round-key (mode:string round:integer)
    (format "{}.{}" [mode round])
  )
  (defun get-mode-round-number-key (mode:string round:integer number:integer)
    (format "{}.{}.{}" [mode round number])
  )
  (defun get-mode-round-account-key (mode:string round:integer account:string)
    (format "{}.{}.{}" [mode round account])
  )
  ...
  (defun vote(mode:string round:integer account:string number:integer amount:decimal want-to-end-current-round:bool)
    ...
    (enforce-current-round mode round)
    ...
    (with-capability (VOTE account)
      ...
      (let* ((round-account-number-key (get-mode-round-account-number-key mode round account number))
        ...
        (add-votes mode round account number amount)
        (if (= true (can-move-to-phase-2 mode round)) (move-to-phase-2 mode round) true)
        (if (and (= cur-phase PHASE_TWO) (and want-to-end-current-round (can-end-current-round mode round account amount))) (end-current-round mode round) true)
      )
    )
  )

  (defun move-to-phase-2(mode:string round:integer)
    (require-capability (MOVE_TO_PHASE_2))
    (with-read round-table (get-mode-round-key mode round) {"voted-amount-list":= voted-amount-list}
      (update round-table (get-mode-round-key mode round) {"phase": PHASE_TWO, "phase-1-voted-total": (sum-list voted-amount-list)})
    )
  )
  ...
  (defun can-end-current-round:bool(mode:string round:integer account:string amount:decimal)
    ...
    (with-read round-table (get-mode-round-key mode round) {"hash":=cur-hash, "phase-1-voted-total":=phase-1-voted-total, "voted-amount-list":=voted-amount-list}
      (let*
        (
  ...
  (defun end-current-round(mode:string round:integer)
    ...
    (burn mode round)
    (update round-table (get-mode-round-key mode round) {"close-time": (get-current-time), "is-closed": true})
    (create-new-round mode (+ round 1))
    (x-vote mode round (+ round 1))
  )
  (defun get-current-time:time()
    (at "block-time" (chain-data))
  )

  (defun create-new-round(mode:string new-round:integer)
    (require-capability (CREATE_ROUND))
    (let (
        (current-round (get-current-round mode))
        (round-key (get-mode-round-key mode new-round))
      )
      ...
      (insert round-table round-key {
          ...
          "phase": PHASE_ONE,
          "hash": (if (= new-round 0) (hash "initial hash") (at "hash" (round-details mode current-round)))
        }
      )
      (update state-table mode {"current-round": new-round})
    )
  )
  (defun enforce-round-closed(mode:string round:integer)
    (with-read round-table (get-mode-round-key mode round) {"is-closed":= is-closed} (enforce (= true is-closed) "round is not closed yet!"))
  )
  (defun claim(mode:string round:integer account:string)
    ; make sure that each acount can claim at most 1 time
    ; make sure that round is already closed
    (enforce-account-owner account)
    (enforce-round-closed mode round)
    (with-default-read user-claim-table (get-mode-round-account-key mode round account) {"is-claimed":false} {"is-claimed":= is-claimed}
      (enforce (= is-claimed false) "already claimed!")
    )
    (let ((win-amount (get-account-win-amount mode round account)))
      ...
      (with-capability (PRIVATE_RESERVE)
        ...
        (insert user-claim-table (get-mode-round-account-key mode round account) {"is-claimed":true, "claim-date": (get-current-time)})
      )
    )
  )
  (defun get-account-win-amount(mode:string round:integer account:string)
    (with-read round-table (get-mode-round-key mode round) {"voted-amount-list":= voted-amount-list}
      (let ((winning-number (amin-list voted-amount-list)))
        (with-default-read votting-position-by-account-table (get-mode-round-account-number-key mode round account winning-number) {"total": 0.0} {"total":= total} total)
      )
    )
  )
  (defun host-claim(mode:string round:integer)
    (with-default-read host-claim-table (get-mode-round-key mode round) {"is-claimed":false} {"is-claimed":= is-claimed}
      (enforce (= is-claimed false) "already claimed!")
    )
    (with-read host-account-table HOST_ACCOUNT_KEY {"account":=account}
      (enforce-account-owner account)
      (enforce-round-closed mode round)
      (let ((amount (get-host-earn-amount mode round)))
        (install-capability (coin.TRANSFER CONTRACT_ACCOUNT account amount))
        (with-capability (PRIVATE_RESERVE)
          (coin.transfer CONTRACT_ACCOUNT account amount)
          (insert host-claim-table (get-mode-round-key mode round) {"is-claimed": true, "claim-date": (get-current-time)})
        )
      )
    )
  )
  (defun add-votes(mode:string round:integer account:string number:integer amount:decimal)
    ...
    (with-read round-table (get-mode-round-key mode round) {"filled-number":= filled-number, "voted-amount-list":= voted-amount-list, "hash":= cur-hash}
      (let*
        (
          (voted-amount (at number voted-amount-list))
        ...
        (update round-table (get-mode-round-key mode round)
          {
  ...
  (defun x-vote(mode:string current-round:integer new-round:integer)
    (require-capability (X_VOTE))
    (let
      (
        (x-winning-amount (get-x-winning-amount mode current-round))
        (x-award (get-x-award mode current-round))
        (number (mod new-round 2))
      )
      (add-votes mode new-round CONTRACT_ACCOUNT number (+ (* x-winning-amount 2) x-award))
    )
  )
  (defun burn(mode:string round: integer)
    ;always burn at the end of each round, so this function can be private
    (require-capability (BURN))
    (let* ((burn-amount (get-burn-amount mode round)))
      (if (> burn-amount 0.0)
        [
  ...
  (defun get-x-award (mode:string round:integer)
    (let ((total-diff (get-total-diff mode round))) (* total-diff X_EARN_RATE))
  )
  (defun get-x-winning-amount(mode:string round:integer)
    (get-account-win-amount mode round CONTRACT_ACCOUNT)
  )
  (defun get-burn-amount (mode:string round:integer)
    (let* ((total-diff (get-total-diff mode round))) (* total-diff BURN_RATE))
  )
  (defun get-host-earn-amount (mode:string round:integer)
    (let* ((total-diff (get-total-diff mode round))) (* total-diff HOUSE_EARN_RATE))
  )
  (defun get-total-diff(mode:string round:integer)
    (let* (
        (voted-amount-list (at "voted-amount-list" (round-details mode round)))
  ...
  (defun round-details(mode:string round:integer)
    (read round-table (get-mode-round-key mode round) ["open-time" "close-time" "filled-number" "is-closed" "voted-amount-list" "phase" "hash" "phase-1-voted-total"])
  )
  (defun get-current-round:integer(mode:string)
    (with-default-read state-table mode {"current-round": -1} {"current-round":=cur-round} cur-round )
  )
